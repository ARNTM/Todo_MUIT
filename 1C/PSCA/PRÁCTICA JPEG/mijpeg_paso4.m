function mijpeg_paso4(imC, nombre_salida)
...
cabecera(626) = floor(alt/256);
cabecera(627) = mod(alt,256);
cabecera(628) = floor(anch/256);
cabecera(629) = mod(anch,256);
...
%para hacerlo de una tacada reordeno la imagen, pongo cada pixel en una columna
n_pixels = anch*alt;
...
buf_bits=[];
DC_anterior=zeros(1,3); % buffers para el coeficiente DC de Y, Cb y Cr
...
for nb_v=0:(alt/8)-1
   for nb_h=0:(anch/8)-1
      for cmp=1:3
         ...
         if verbose_level >= 1 nb_h,nb_v, cmp, end
         
         %debug
         if verbose_level >= 3
            display('COEFICIENTE DE CONTINUA')
         end
         B = imYCbCr(nb_v*8+1:nb_v*8+8,nb_h*8+1:nb_h*8+8, cmp);
         D = dct2(B);

         if cmp == 1  % en el paso 1 solo la luminancia

            % REALIZAR  PASO 1    
         	%   dividir por la tabla de cuantificación
            	%%%%%% XXXXXXXXXXXXXXXXXXX            
            ...
            %   llamar "DC_diferencial" al resultado
                ...
                DC_diferencial = D_div(1,1) - DC_anterior(1);
                DC_anterior(1) = D_div(1,1);
         ...
         else % en el paso 1 la croma ponemos end of block
               %bits_DC=char(HUFF_DC_CROMA(1)); %codigo 0, indice 1 de la tabla  
                              %bits_DC=char(HUFF_DC_CROMA(1)); %codigo 0, indice 1 de la tabla 
               D_div = round(D./qtc);
               DC_diferencial = D_div(1,1) - DC_anterior(cmp);
               DC_anterior(cmp) = D_div(1,1);
         	    ...
         	    bits_DC_nbits = char(HUFF_DC_CROMA(nbits+1));  % si no se pone char el tipo es cell y no funciona bien el strcat
         ...
         %PARA PROBAR SI EL CODIGO FUNCIONA BIEN 
         % descomenta las lineas una a una y quita
         % el punto y coma de la linea: bits_AC; error('finaliza prueba');
         % que hay despues del end del 
         % for ii=1:n_runlen
         
         % runlen = [-1 0]  % ningun runlen
         % runlen = [3 4; -1 0]   %un runlen
         % runlen = [17 4; -1 0]   %un runlen de mas de 15 ceros
         % runlen = [32 4; -1 0]   %un runlen con 32 ceros
         % runlen = [0 3; 32 4; -1 0] % un runlen normal + uno largo
         
         
         [n_runlen, dummy]=size(runlen); % numero de runlengths
         for ii=1:n_runlen
             number_of_zeros = runlen(ii,1);
             coef = runlen(ii,2);

             %  INICIO A REALIZAR  PASO 4
             %%%%%%%%%%%%%%%%%%%%%%%
         	%%%%%%%%%%%%%%%%%%%%%%%

           	% BREVE RECORDATORIO DE LAS TAREAS A REALIZAR:
			%  si number_of_zeros es -1 hay que poner EOB, de lo contrario
			% hay que confeccionar el par (numero de ceros, numero de bits del coeficiente) y generar el código correspondiente. Luego los bits adicionales del coeficiente. 
			% si number_of_zeros es mayor que 15 hay que poner el código adecuado cada 16 ceros.
			% recordad que los tablas a utilizar serán distintas si se
			% trata de coeficientes de la componente 1 (luminancia) que si se tratan de las componentes 2 y 3 (crominancia)
            %   ver DIAGRAMA DE FLUJO en el enunciado de la practica y
            %   tabla con ejemplos de comprobacion
            ...
            if number_of_zeros == -1
                ...
                while (number_of_zeros > 15)
                    if cmp == 1
                        ...
                        bits_AC = strcat(bits_AC, char(HUFF_AC_CROMA(241)));
                        number_of_zeros = number_of_zeros - 16;
                    end
                end
